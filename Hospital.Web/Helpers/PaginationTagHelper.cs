...
namespace Hospital.Web.Helpers
{
    public class PaginationTagHelper : TagHelper
    {
        ...
        private void ClosingHtml(StringBuilder content)
        {
            ...
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?pagenumber={PageCount}&pagesize={PageRange}'>{PageLast}</a></li>");
        ...
        private void PageNumberUpperPageRange(StringBuilder content)
        {
            for (int currentPage = PageCount - (2 * PageRange); currentPage < PageCount + 1; currentPage++)
            {
                ...
                content.Append($"<li class='page-item {active}'><a class='page-link'href='{PageTarget}?pagenumber={currentPage}&pagesize={PageRange}'>{currentPage}</a></li>");
            }
        }
        ...
        private void InitPaginationHtml(StringBuilder content)
        {
            ...
            if (PageNumber != 1)
            {
                content.Append($"<li class='page-item'><a class='page-link' href='{PageTarget}?pagenumber={1}&pagesize={6}'>{PageFirst}</a></li>");
            }
        }
